...
// El botón del carrito ("Comprar", id="btnComprar") abre el carrito de Shopify con la cantidad seleccionada
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // SOLO el botón del carrito (btnComprar) abre el carrito de Shopify con la cantidad seleccionada
  if(btnComprarCarrito && inputCantidad){
    btnComprarCarrito.addEventListener('click', function(e){
      e.preventDefault();
      let qty = parseInt(inputCantidad.value, 10) || 1;
      const url = `https://${SHOP_DOMAIN}/cart/${DEFAULT_VARIANT_ID}:${qty}`;
      window.open(url, '_blank', 'noopener');
    });
  }
